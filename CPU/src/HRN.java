...
public class HRN extends SchedulerManage {
    @Override
    public void process() {
        ...
        while (!rows.isEmpty()) {
            ...
            Collections.sort(availableRows, (Object o1, Object o2) -> {
                if (((Row) o1).getHRNpl()== ((Row) o2).getHRNpl()) {
                    return 0;
                }
                else if (((Row) o1).getHRNpl() < ((Row) o2).getHRNpl()) {
                    return -1;
                }
                else {
                    return 1;
            ...
            this.getTimeflow().add(new Event(row.getProcessName(), time, time + row.getBurstTime(),row.getColor()));
